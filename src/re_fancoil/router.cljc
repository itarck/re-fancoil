(ns re-fancoil.router
  (:require
   [re-fancoil.lib.interop :refer [after-render empty-queue next-tick]]
   [re-fancoil.global.trace   :as trace :include-macros true]
   [re-fancoil.global.loggers     :refer [console]]
   [re-fancoil.events  :refer [handle-event]]
...
(deftype EventQueue [#?(:cljs ^:mutable fsm-state               :clj ^:volatile-mutable fsm-state)
                     ...
                     #?(:cljs ^:mutable registrar               :clj ^:volatile-mutable registrar)]
  IEventQueue
  ...
  (-process-1st-event-in-queue
    ...
    (let [event-v (peek queue)]
      (try
        (handle-event registrar event-v)
        (set! queue (pop queue))
...
;;
#_(def event-queue (->EventQueue :idle empty-queue {}))
...
(defn dispatch-sync
  [{:keys [event-queue registrar]} event-v]
  (handle-event registrar event-v)
  (-call-post-event-callbacks event-queue event-v)  ;; slightly ugly hack. Run the registered post event callbacks.
  nil)                                              ;; Ensure nil return. See https://github.com/day8/re-frame/wiki/Beware-Returning-False


(defmethod ig/init-key :re-fancoil/router
  [_ {:keys [registrar]}]
  (let [self {:event-queue (->EventQueue :idle empty-queue {} registrar)
              :registrar registrar}
        dispatch-fn (partial dispatch self)
        dispatch-sync-fn (partial dispatch-sync self)]
    (-> self
        (assoc :dispatch-fn dispatch-fn)
        (assoc :dispatch-sync-fn dispatch-sync-fn))))