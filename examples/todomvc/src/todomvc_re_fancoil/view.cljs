...
(defmethod ig/init-key :todomvc-re-fancoil.view/root-view
  [_ {:keys [router subs]}]
  (let [dispatch-fn (get-in router [:dispatch-fn])
        subsribe-fn (get-in subs [:subscribe-fn])]
    [todo-app {:dispatch dispatch-fn
               :subscribe subsribe-fn}]))
